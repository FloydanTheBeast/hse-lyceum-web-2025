type Markdown = string;

export type Slide = {
  title: string;
  description: Markdown;
  grade: 10 | 11;
  tags?: string[];
  path: string;
};

export const SLIDES: Slide[] = [
  {
    title: "Вводное занятие",
    description: `
- Содержание курса
- HTML
  - Структура документа
  - Элементы и тэги
  - Семантическая вёрстка
  - Поток документа
- CSS
  - Блочная модель
  - Селекторы и их виды
  - Специфичность селекторов
  - Каскадность
  - Наследование стилей
- JavaScript - что за язык и его основные характеристики
    `,
    grade: 10,
    tags: ["Frontend", "HTML", "CSS", "JavaScript"],
    path: "10-1",
  },
  {
    title: "Основы фронтенд-разработки",
    description: `
- CSS
  - Позиционирование
  - Наложение слоёв (\`z-index\`)
  - Флексбокс (Flexbox)
  - Единицы измерения
  - Функции
  - Переменные
- JavaScript
  - Область видимости
  - Поднятие (Hoisting)
  - Различие \`var\`, \`let\`, и \`const\`
    `,
    grade: 10,
    tags: ["Frontend", "CSS", "JavaScript"],
    path: "10-2",
  },
  {
    title: "CSS. Основы работы JavaScript в браузере",
    description: `
- CSS
  - Грид (Grid)
  - Псевдоэлементы
  - Псевдоклассы
  - Трансформации (Свойство \`transform\`)
  - Переходы (Свойство \`transition\`)
- JavaScript
  - Браузерное окружение
  - Объектная модель документ (DOM)
    `,
    grade: 10,
    tags: ["Frontend", "CSS", "JavaScript"],
    path: "10-3",
  },
  {
    title: "JavaScript: объекты, функции, классы и событийная модель",
    description: `
- Объекты
  - Как хранятся
  - Геттеры и сеттеры
- Ключевое слово \`this\`
  - Явное и неявное связывание
  - Алгоритм определения
- Функции-конструкторы
- IIFE
- Прототипы и наследование
- Классы
    - Приватные поля и методы
    - Защищённые поля и методы
    - Статические поля и методы
- Строгий режим (Strict mode)
- Событийная модель
    - Погружение и всплытие событий
    `,
    grade: 10,
    tags: ["Frontend", "JavaScript", "ООП"],
    path: "10-4",
  },
  {
    title: "Git. JS: делегирование событий, структуры данных. Адаптивность.",
    description: `
- Git
- JavaScript
  - Делегирование событий
  - Встроенные структуры данных - \`Set\`, \`Map\`
- Адаптивность веб-страниц
    `,
    grade: 10,
    tags: ["Git", "Frontend", "JavaScript", "CSS"],
    path: "10-5",
  },
  {
    title: "CSS: анимации и БЭМ. JS: основы Web API и асинхронность",
    description: `
- CSS
  - Анимации
  - Методология БЭМ
- Основы Web API
- Асинхронность в JS
  - Коллбэки
  - Объект \`Promise\`
  - \`async\`, \`await\`
    `,
    grade: 10,
    tags: ["Frontend", "JavaScript", "CSS"],
    path: "10-6",
  },
  {
    title: "JS - Забытые темы",
    description: `
- Логические операторы
- Оператор нулевого слияния и присваивания
- Опциональные цепочки
- Spread-оператор
- Деструктуризация массивов и объктов
- Стилизация при помощи JS
- JSON
- Копирование ссылочных типов
- Хранение данных в браузере - \`localStorage\`, \`sessionStorage\`, Cookie
- Пользовательские события
- Основы сетевых запросов
    `,
    grade: 10,
    tags: ["Frontend", "JavaScript", "Компьютерные сети"],
    path: "10-7",
  },
  {
    title: "HTML: async, defer, формы. JS: location, history, маршрутизация",
    description: `
- HTML
  - \`async\`, \`defer\`
  - Формы: основные элементы, валидация, обработка при помощи JS
- JS
  - Регулярные выражения
  - Объект истории - \`window.history\`
  - Основы клиентской маршрутизации
    `,
    grade: 10,
    tags: ["Frontend", "HTML", "JavaScript"],
    path: "10-8",
  },
  {
    title: "Углублённый JS",
    description: `
- Функции и объекты первого класса
- Замыкания (Closure)
- Лексическая область видимости и лексическое окружение
- Событийный цикл (Event Loop)
- Основы JS-модулей и динамический импорт
    `,
    grade: 10,
    tags: ["Frontend", "JavaScript"],
    path: "10-9",
  },
  {
    title: "Внешние зависимости и NPM. Основы сборки приложений",
    description: `
- Что такое внешние зависимости
- Основы работы с NPM, аналогичные инструменты (yarn, pnpm, bun)
- \`package.json\`
- \`package-lock.json\`
- Версионирование пакетов, SemVer
- NPX
- Основы сборки приложений
    - Зачем это нужно
    - Основы Webpack
    - Основы Vite
    `,
    grade: 10,
    tags: ["Frontend", "JavaScript", "Инструменты", "Зависимости"],
    path: "10-10",
  },
  {
    title: "Повторение пройденного",
    description: `
Не изучаем ничего нового. Повторяем всё (или почти всё), что успели пройти на прошлых занятиях
    `,
    grade: 10,
    tags: ["Frontend", "Практика"],
    path: "10-11",
  },
  {
    title: "UNIX, основы работы в терминале",
    description: `
- Что такое UNIX?
- WSL
- Потоки данных в UNIX
- Работа в терминале
  - Навигация по файловой системе
  - Работа с файлами и директориями
  - Просмотр и редактирование файлов
  - Поиск по файлам и тексту
  - Шаблоны поиска
  - Работа с процессами
  - Пайпы и перенаправление потоков
  - Логические операторы и цепочки команд
  - Пакетные менеджеры (Homebrew, Apt, Chocolatey)
  - Переменные окружения
  - Конфигурация оболочки
    `,
    grade: 10,
    tags: ["Инструменты", "Терминал"],
    path: "10-12",
  },
  {
    title: "React - продолжение",
    description: `
- Мемоизация в программировании
- \`useMemo\`
- \`useCallback\`
- \`React.memo\`
- Компоненты высшего порядка (HOC)
- \`useRef\`
- Context API
- Пользовательские хуки
- React Router
  - Создание маршрутизатора
  - Навигация
  - Основные хуки
- Подходы к стилизация компонентов
- React Developer Tools
    `,
    grade: 10,
    tags: ["Frontend", "JavaScript", "React", "Машрутизация"],
    path: "10-13",
  },
  {
    title: "React - продолжение",
    description: `
- Мемоизация в программировании
- \`useMemo\`
- \`useCallback\`
- \`React.memo\`
- Компоненты высшего порядка (HOC)
- \`useRef\`
- Context API
- Пользовательские хуки
- React Router
  - Создание маршрутизатора
  - Навигация
  - Основные хуки
- Подходы к стилизация компонентов
- React Developer Tools
    `,
    grade: 10,
    tags: ["Frontend", "JavaScript", "React", "Машрутизация"],
    path: "10-13",
  },
  {
    title: "Инструменты качества кода и язык TypeScript",
    description: `
- Качество кода
  - Каким образом поддерживается
  - Форматировщики
  - Линтеры
  - Статические анализатор
  - Git-хуки и \`pre-commit\`
  - Conventional Commits
- TypeScript
  - Базовые типы
  - Автовыведение типов
  - Типы и интерфейсы, чем различаются
  - Операторы \`keyof\` и \`typeof\`
  - Объединённые типы (Union types)
  - Пересечённые типы (Intersection types)
  - Слияние деклараций (Declaration merging)
  - Сужение типов (Type narrowing)
  - Защитники типов (Type guards)
  - Операторы \`as\` и \`satisfies\`
  - Типизация объектов и функций
  - Работа с \`null\` и \`undefined\`
  - Перечисления (\`enum\`)
  - Обобщённые типы (Generics)
  - Условные типы
  - Mapped types
  - Оператор \`infer\`
  - Утилитарные типы
  - Работа с классами
  - Использование в React
    `,
    grade: 10,
    tags: ["Frontend", "TypeScript", "Качество кода", "React"],
    path: "10-14",
  },
  {
    title: "Вводное занятие",
    description: `
- Содержание курса
- Компьютерные сети
  - Модель OSI
  - Стек протоколов TCP/IP
  - Адресация
  - Протоколы
    - DNS
    - UDP
    - HTTP
    `,
    grade: 11,
    tags: ["Компьютерные сети"],
    path: "11-1",
  },
  {
    title: "Python - дополнительные главы",
    description: `
- Модули и пакеты
- Работа с внешними зависимостями
- Виртуальные среды
- Управление версией интерпретатора (\`pyenv\`)
- Аннотации типов (Type hints)
  - Пользовательские типы
  - Обобщённые типы
  - Аннотации во время исполнения
- Асинхронное программирование
    `,
    grade: 11,
    tags: ["Python", "Инструменты", "Зависимости"],
    path: "11-2",
  },
  {
    title: "Бэкенд-фреймворки",
    description: `
- Разница между библиотекой и фреймворком
- Сравнение популярные веб-фреймворков
- Основы FastAPI
- Декораторы
    `,
    grade: 11,
    tags: ["Backend", "FastAPI"],
    path: "11-3",
  },
  {
    title: "Основы проектирования API, обзор архитектурных подходов",
    description: `
- Что такое API, интерфейс и сервис
- Разновидности API
- REST API
  - Назначение
  - Принципы
  - HTTP-методы
- Инструменты для тестирования API
- GraphQL - крактий обзор
- Сравнение архитектурных подходов
- Регулярные выражения
    `,
    grade: 11,
    tags: ["Backend", "API", "FastAPI", "Проектирование"],
    path: "11-4",
  },
  {
    title: "CORS. Документирование.",
    description: `
- Мехазинм безопасности CORS
- Документация
  - OpenAPI и инструменты (Swagger, Redoc)
  - JSDoc
  - Python docstrings
    `,
    grade: 11,
    tags: ["Backend", "Безопаность", "Документация"],
    path: "11-5",
  },
  {
    title: "Качество кода. Введение в базы данных и SQL.",
    description: `
- Качество кода
  - Каким образом поддерживается
  - Форматировщики
  - Линтеры
  - Статические анализатор
  - Git-хуки и \`pre-commit\`
  - Conventional Commits
  - \`pyproject.toml\`
  - Lock-файлы
- Базы данных
  - Для чего нужны и какие бывают
  - Реляционная модель данных
  - ER-диаграммы
- SQL
  - Что это такое
  - Типы данных
  - DDL
  - Ключи и связи таблиц
    `,
    grade: 11,
    tags: ["Backend", "Качество кода", "Базы данных", "SQL", "Git"],
    path: "11-6",
  },
  {
    title: "SQL: DDL, Constraints, DML, DQL",
    description: `
- Язык определения данных (DDL)
  - Ограничения (Constraints)
- Язык манипуляции данными (DML)
- DQL
  - Опертор \`SELECT\`
  - Сортировка - оператор \`ORDER BY\`
  - Объединение таблиц (оператор \`JOIN\`) и его виды
  - Группировка и агрегация данных - оператор \`GROUP BY\`
  - Подзапросы (Subqueries)
    `,
    grade: 11,
    tags: ["Backend", "SQL"],
    path: "11-7",
  },
  {
    title:
      "SQL: Рекурсивные запросы, оконные функции, представления, индексы. Контейнеризация: Docker и Docker-compose",
    description: `
- SQL
  - Рекурсивные запросы
  - Оконные функции
  - Представления (SQL View)
  - Индексы, их виды, B и B+-деревья
- Контейнеризация
  - Docker: конфигурация, тома, и сети
  - Docker-compose
    `,
    grade: 11,
    tags: ["Backend", "SQL", "DevOps", "Контейнеризация"],
    path: "11-8",
  },
  {
    title: "Основы PostgreSQL. Дравйверы и ORM",
    description: `
- PostgreSQL
  - Схемы
  - Роли и права
  - Основы \`EXPLAIN ANALYZE\`
- Взамодействие с БД в программах
  - Драйверы
  - ORM, основы SQLAlchemy
    `,
    grade: 11,
    tags: ["Backend", "Базы данных", "SQL", "Python"],
    path: "11-9",
  },
  {
    title: "Введение в React. Сборщики",
    description: `
- Что такое React и для чего нужен
- JSX
- Компоненты
- Транспиляция
- Пропы (Props)
- Состояние (State)
- Условный рендеринг
- Рендеринг списков
- Жизненный цикл компонентов
- Хуки
  - Что это такое
  - Правила хуков
  - \`useState\`
  - \`useEffect\`
    `,
    grade: 11,
    tags: ["Frontend", "React"],
    path: "11-10",
  },
  {
    title: "Авторизация",
    description: `
- Регистрация, идентификация, аутентификация и авторизация: в чём разница?
- Виды аутентификации
  - Stateful-аутентификация (сессионная)
  - Что такое Cookie?
  - Stateless аутентификация (при помощи токена)
- JWT
  - Алгоритм проверки подписи
  - Токен доступа и обновления (Access and Refresh Token)
- Хранение паролей в базе данных - хэширование с солью
    `,
    grade: 11,
    tags: ["Backend", "Авторизация", "Базы данных"],
    path: "11-11",
  },
  {
    title: "Тестирование",
    description: `
- Что такое тестирование ПО?
- Виды/классификация тестирования
- Пирамида тестирования и другие модели
- Инструменты тестирования
- Кто такие QA-инженеры?
- Роль разработчиков в тестировании
- Глоссарий
- Основы методологии Test-Driven Development (TDD)
- Мутационное тестирование
    `,
    grade: 11,
    tags: ["Тестирование", "Инструменты", "Методологии"],
    path: "11-12",
  },
  {
    title: "Управление проектом и продуктом",
    description: `
- Различие проекта и продукта, зачем нужно ими управлять?
- Роли менеджера проекта (project manager) и менеджера продукта (product manager)
- Жизенный цикл разработки ПО (SDLC)
- Этап планирования
- Этап анализа требований
- Виды требований: функциональные и нефункциональные
- SLI, SLO и SLA
- Проектирование и дизайн
- Этап тестирования
- Этап релиза
- Этап эксплуатации и поддержки
- Работа с рисками
- Метрики в программном продукте: что это такое и зачем нужны?
- Что такое мониторинг?
- Что такое логирование?
- Основы CI/CD
    `,
    grade: 11,
    tags: ["Управление разработкой", "Требования", "CI/CD"],
    path: "11-13",
  },
];

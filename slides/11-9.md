...
<div class="mx-auto w-60%">
  <img src="https://www.pgadmin.org/static/COMPILED/assets/img/screenshots/pgadmin4-history.png" width="100%" />
  <figcaption><b>PgAdmin</b> - графический клиент для postgres</figcaption>
</div>
...
> **Миграции базы данных** — это механизм управления изменениями структуры БД (схемы, schema) с сохранением её истории и возможностью откатывать изменения (по аналогии с git)

### Зачем нужны?

- **Контроль версий схемы БД** - структура базы развивается вместе с кодом
- **Работа в команде** - синхронизация изменений между разработчиками
- **Автоматическое развертывание** - одинаковая структура БД на dev / test / prod
- **Откат изменений** - возможность вернуться к предыдущей версии схемы

### Как работают?

- Создают скрипты изменений схемы - обычно это просто программа языке вашего проекта
- Ведут историю версий базы данных - в отдельной таблице БД хранят состояние миграций
- Применяют и откатывают изменения, то есть позволяют запустить скрипты для изменений
- Интегрируются с ORM и CI/CD

---

# Миграции БД

На примере **[Alembic](https://github.com/sqlalchemy/alembic)**

> **Alembic** — инструмент управления миграциями для Python-проектов, использующих **SQLAlchemy** ORM

### Как работает

- Разработчик изменяет модели данных
- Alembic генерирует файл миграции
...
- `alembic downgrade` - откатить изменения
...
Postgres + SQLAlchemy + Alembic + Docker Compose
...
- Установите PostgreSQL и, если хотите, PgAdmin (можно использовать docker)
- Создайте БД из [SQL файла](https://github.com/neondatabase-labs/postgres-sample-dbs/blob/main/chinook.sql)
  ...
  # либо при помощи утилиты pg_restore
  ```
...
- Опишите конфигурацию **docker-compose**, чтобы приложение, БД и PgAdmin работали в контейнерах
...
- [Habr - Асинхронный SQLAlchemy 2: простой пошаговый гайд по настройке, моделям, связям и миграциям с использованием Alembic](https://habr.com/ru/companies/amvera/articles/849836/)